...
class chat():
    def __init__(self):
        ...
        chat_text.insert(tk.END, 'find all servers? [yes to find servers,no to write the ip yourself]' + "\n")
    ...
    def game(self, msg):
        ...
        for msgg in msglst:
            ...
            if msgg.find("server: @you are drawing") == 0:
                print("drawing")
                playing = True
                thread = threading.Thread(target=handDetector.trackhands)
                thread.start()
            ...
            if Imsg.find('@px2') == 1:
                ...
                playing = False
                self.add_message_to_ui("playing = " + str(playing))
    ...
    def send_message(self):
        global findingip,client,serverip, choose,ip,write_ip,ips
        message = message_entry.get()
        self.add_message_to_ui("You: " + message)
        ...
        if choose:
            if str(message).lower() == 'yes':
                ...
                self.add_message_to_ui('write the server ip:')
                choose = False
                write_ip = True
            else:
                self.add_message_to_ui('write yes or no only')
        elif write_ip:
            try:
                ip = str(message)
                print(ip)
                ...
                if result == 0:
                    ...
                    if data == "@hello!!":
                        ...
                        self.add_message_to_ui(f"connected to - ('{serverip}', 7835), write !ready to start and !help for more commands")
                        thread = threading.Thread(target=self.receive_messages)
                        thread.start()
                        write_ip = False
            except:
                self.add_message_to_ui("couldn't connect to the server, check the ip and try again")
        elif findingip:
            if str(message).isdigit():
                if int(message) < len(ips):
                    ...
                    self.add_message_to_ui('too big')
            else:
                self.add_message_to_ui('this is not a int')
        else:
            if message.find('@') == 0:
                message = message[1:]
            if message == "!help":
                self.add_message_to_ui("commands help:")
                self.add_message_to_ui("- !ready : start the game")
                self.add_message_to_ui("- !time ___ : set the time for each turn")
                self.add_message_to_ui("- !rounds ___ : set the number of rounds")
                self.add_message_to_ui("- !exit : exit the program")
            else:
...
class findservers():
    def __init__(self):
        global findingip, ips
        chat.add_message_to_ui(chat , "finding servers... (might take some time)")
        time.sleep(1)  
        ips = self.find_ips_on_port(7835)
        serverip = '127.0.0.1'
        findingip = False
        print('server ips: ' + str(ips))
        if len(ips) == 0:
            chat.add_message_to_ui(chat,"found 0 servers...")
        elif len(ips) == 1:
            serverip = ips[0]
            client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            client.connect((serverip, 7835))
            chat.add_message_to_ui(chat,f"connected to - ('{serverip}', 7835), write !ready to start")
            thread = threading.Thread(target=chat.receive_messages)
            thread.start()
        else:
            chat.add_message_to_ui(chat,'server ips: (write the server number to choose)')
            for i in range(len(ips)):
                chat.add_message_to_ui(chat,f'{i + 1} - {ips[i]}')
            findingip = True
    ...
    def find_ips_on_port(self,port):
        ...
        for i in range(1, 256):
            for j in range(1, 256):
                ...
                t = threading.Thread(target=self.check_ip, args=(ip, port))
                t.start()
                threads.append(t)
...
chat_ui_thread = threading.Thread(target=chat)
