...
def trackhands():
    ...
    while playing:
        ...
        cv2.putText(img, 'time left: ' + str(timer - int(cTime - start)), (10, int(width / 5)), cv2.FONT_HERSHEY_PLAIN,
                    1,
                    (0, 0, 0), 2)
...
def send_message():
    global findingip,client,serverip
...
def find_ips_on_port(port):
    ...
    for i in range(1, 256):
        for j in range(1, 256):
            # Use first two octets of local IP and last two octets from loop
...
time.sleep(1)
add_message_to_ui("finding servers...")
